...
After participating in the Testathon and reflecting on the next steps for the game, we reached the conclusion that enabling the game for mobile devices would be our next big challenge. We start by researching what would be the main implications and feasibility concerns, finding the following key points that end up being our work plan.
Input Method Changes. Our game currently uses keyboard input (e.g., arrow keys, spacebar) to control gameplay. Mobile devices lack a physical keyboard so we will need to adapt controls to touch interactions.
Responsive Design and Layout. Our game was optimized for desktop resolution with fixed canvas sizes and positioning. Mobile screens are smaller and come in different aspect ratios. The layout must adapt to these constraints.
Performance and Optimization: Our game was built for desktop environments where performance constraints are generally less severe. Mobile devices have less processing power and memory compared to desktops. Graphics, animations, and collision detection may need optimization.
Testing Across Devices. Our game has been tested mainly on desktop browsers. Mobile devices vary widely in terms of browser capabilities, touch responsiveness, and performance.
